...
public class ModeloLoginImpl implements ModeloLogin {
	...
	public ModeloLoginImpl() {
		/*
		 * Cargamos los usuarios del servidor de B.D. del archivo de propiedades en la
		 * tabla hash "usuarios"
		 */
		...
		try {
			...
			while (enumeration.hasMoreElements()) {
				...
				String[] label = key.split("[.]"); // el punto es un caracter especial que significa cualquier caracter
													// como en SQL entonces hay que escapar o utilizar clase caracter
													// con []
				...
				if (label[1].equals("username"))
					user.setUsername(valor);
				if (label[1].equals("password"))
					user.setPassword(valor);
				if (label[1].equals("displayname"))
					user.setDisplayname(valor);
