...
public class VentanaEmpleadoImpl extends JFrame implements VentanaEmpleado {
	...
	public void mostrarEmpleadoLogueado(EmpleadoBean empleado) {
		logger.info("Se registra al empleado {}, {} con legajo {}", empleado.getApellido(), empleado.getNombre(),
				empleado.getLegajo());

		this.lblEmpleado.setText("Legajo: " + String.valueOf(empleado.getLegajo()) + " - " + empleado.getApellido()
				+ ", " + empleado.getNombre());
	...
	public void mostrarVuelosDisponiblesSoloIda(ArrayList<InstanciaVueloBean> vuelosDisponibles) {
		logger.info("Se solicita a la ventana mostrar la información de {} vuelos para viaje de ida",
		 ...
		 */
		this.lblVuelosDisponiblesIDA
				.setText("Vuelos desde " + ((UbicacionesBean) this.cmbOrigen.getSelectedItem()).getCiudad() + " hasta "
						+ ((UbicacionesBean) this.cmbDestino.getSelectedItem()).getCiudad() + " disponibles para "
						+ this.dateChooserDesde.getDate().toString());

		this.poblarTablaVuelosDisponibles(this.modeloTablaVuelosDisponiblesIda, vuelosDisponibles);
		/*
		 * Actualiza la variable interna de vuelos disponibles para cuando debe pasarle
		 * info al controlador que no está presente en la tabla (para no poner columnas
		 * ocultas)
		 */
	...
	public void mostrarVuelosDisponiblesVuelta(ArrayList<InstanciaVueloBean> vuelosDisponiblesIda,
		...
		logger.info("Se solicita a la ventana mostrar la información de {} vuelos para viaje de ida y vuelta",
				vuelosDisponiblesIda.size());
		/*
		 * Se cambia la pantalla mostrada
		 */
		 ...
		 * Poblar los componentes
		 */
		this.lblVuelosDisponiblesIDAenIDAVUELTA
				.setText("Vuelos desde " + ((UbicacionesBean) this.cmbOrigen.getSelectedItem()).getCiudad() + " hasta "
						+ ((UbicacionesBean) this.cmbDestino.getSelectedItem()).getCiudad() + " disponibles para "
						+ this.dateChooserDesde.getDate().toString());

		this.lblVuelosDisponiblesVUELTAenIDAVUELTA
				.setText("Vuelos desde " + ((UbicacionesBean) this.cmbDestino.getSelectedItem()).getCiudad() + " hasta "
						+ ((UbicacionesBean) this.cmbOrigen.getSelectedItem()).getCiudad() + " disponibles para "
						+ this.dateChooserHasta.getDate().toString());
	...
	private void poblarTablaVuelosDisponibles(DefaultTableModel modelo,
			ArrayList<InstanciaVueloBean> vuelosDisponibles) {
		...
		for (InstanciaVueloBean vuelo : vuelosDisponibles) {
			...
			fila[modelo.findColumn(VentanaEmpleado.TABLA_VUELOS_DISPONIBLES_NRO_VUELO)] = String
					.valueOf(vuelo.getNroVuelo());
			fila[modelo.findColumn(VentanaEmpleado.TABLA_VUELOS_DISPONIBLES_AEROPUERTO_SALIDA)] = String
					.valueOf(vuelo.getAeropuertoSalida().getNombre());
			fila[modelo.findColumn(VentanaEmpleado.TABLA_VUELOS_DISPONIBLES_HORA_SALIDA)] = String
					.valueOf(vuelo.getHoraSalida().toString());
			fila[modelo.findColumn(VentanaEmpleado.TABLA_VUELOS_DISPONIBLES_AEROPUERTO_LLEGADA)] = String
					.valueOf(vuelo.getAeropuertoLlegada().getNombre());
			fila[modelo.findColumn(VentanaEmpleado.TABLA_VUELOS_DISPONIBLES_HORA_LLEGADA)] = String
					.valueOf(vuelo.getHoraLlegada().toString());
			fila[modelo.findColumn(VentanaEmpleado.TABLA_VUELOS_DISPONIBLES_MODELO_AVION)] = String
					.valueOf(vuelo.getModelo());
			fila[modelo.findColumn(VentanaEmpleado.TABLA_VUELOS_DISPONIBLES_TIEMPO_ESTIMADO)] = String
					.valueOf(vuelo.getTiempoEstimado().toString());
	...
	private void poblarTablaDetalleVuelosDisponibles(DefaultTableModel modelo, ArrayList<DetalleVueloBean> detalles) {
		...
		for (DetalleVueloBean detalle : detalles) {
			...
			fila[modelo.findColumn(VentanaEmpleado.TABLA_DETALLE_VUELOS_DISPONIBLES_CLASE)] = String
					.valueOf(detalle.getClase());
			fila[modelo.findColumn(VentanaEmpleado.TABLA_DETALLE_VUELOS_DISPONIBLES_PRECIO)] = String
					.valueOf(detalle.getPrecio());
			fila[modelo.findColumn(VentanaEmpleado.TABLA_DETALLE_VUELOS_DISPONIBLES_ASIENTOS_DISPONIBLES)] = String
					.valueOf(detalle.getAsientosDisponibles());
	...
	public void mostrarSolicitudReservaSoloIda(ArrayList<String> tiposDocumento, InstanciaVueloBean vuelo,
		...
		lblSeleccionSoloIda.setText("Vuelo desde " + ((UbicacionesBean) this.cmbOrigen.getSelectedItem()).getCiudad()
				+ " hasta " + ((UbicacionesBean) this.cmbDestino.getSelectedItem()).getCiudad() + " para "
				+ this.dateChooserDesde.getDate().toString());
	...
	public void mostrarSolicitudReservaIdaVuelta(ArrayList<String> tiposDocumento, InstanciaVueloBean vueloIda,
		...
		lblSeleccionIda.setText("Vuelo de ida desde " + ((UbicacionesBean) this.cmbOrigen.getSelectedItem()).getCiudad()
				...
				+ this.dateChooserDesde.getDate().toString());
		lblSeleccionVuelta
				.setText("Vuelo retorno desde " + ((UbicacionesBean) this.cmbDestino.getSelectedItem()).getCiudad()
						+ " hasta " + ((UbicacionesBean) this.cmbOrigen.getSelectedItem()).getCiudad() + " para "
						+ this.dateChooserHasta.getDate().toString());
	...
	public void mostrarResultadoReservaSoloIda(ReservaBean reserva) {
		...
		this.lblResultadoSeleccionSoloIda.setText("Vuelo desde " + origen.getCiudad() + " hasta " + destino.getCiudad()
	...
	public void mostrarResultadoReservaIdaVuelta(ReservaBean reserva) {
		...
		this.lblResultadoSeleccionIda.setText("Vuelo desde " + origenIda.getCiudad() + " hasta "
				+ destinoIda.getCiudad() + " para " + vueloIda.getFechaVuelo().toString());
		...
		this.lblResultadoSeleccionVuelta.setText("Vuelo desde " + origenVuelta.getCiudad() + " hasta "
				+ destinoVuelta.getCiudad() + " para " + vueloVuelta.getFechaVuelo().toString());
	...
	/*
	 * Verifica que la reserva tenga todos los objetos ingresados, si no valida tira
	 * excepcion con mensaje a mostrar
	 */
	...
	/*
	 * Listas obtenidas del controlador que guardan los objetos en su clase
	 * original.
	 * 
	...
	private void inicializar() {
		...
		/*
		 * Se evita el comportamiento por defecto que hace un dispose de la ventana y
		 * cierra, porque se requiere que el controlador se haga cargo de cerrar todo,
		 * para cerrar las conexiones a la BD y luego las ventanas.
		 */
	...
	private void registrarEventos() {
		...
		this.getFechaDesde().getDateEditor().addPropertyChangeListener(new PropertyChangeListener() {
			/*
			 * Evento que escucha cuando cambia la fecha desde para impedir que la fecha
			 * hasta sea anterior
			 */
			...
			public void propertyChange(PropertyChangeEvent evt) {
				if (evt.getPropertyName().equals("date")) {
					if (getFechaDesde().getDate() != null && getFechaHasta().getDate() != null
							&& getFechaDesde().getDate().compareTo(getFechaHasta().getDate()) > 0) {
						logger.debug("Fecha desde es mayor a fecha hasta");
						getFechaHasta().setDate(null);
					}
					// Setea la fecha hasta para que no pueda seleccionar un valor menor a fecha
					// desde
		...
		this.getBtnBuscarVuelosDisponibles().addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				controlador.buscarVuelosDisponibles(isIdaVuelta(), getFechaDesde().getDate(), getFechaHasta().getDate(),
	...
	private Component crearPanelVuelosDisponibles() {
		...
		this.modeloTablaDetalleVuelosDisponiblesIda = this
				.inicializarModelo(this.getColumnasTablaDetalleVuelosDisponibles());
		this.modeloTablaVuelosDisponiblesVuelta = this.inicializarModelo(this.getColumnasTablaVuelosDisponibles());
		this.modeloTablaDetalleVuelosDisponiblesVuelta = this
				.inicializarModelo(this.getColumnasTablaDetalleVuelosDisponibles());
		...
		this.panelVuelosDisponibles.add(this.crearPanelResultadoVueloIda(),
				VentanaEmpleado.VUELOS_DISPONIBLES_BUSQUEDA_RESULTADO_SOLO_IDA);
		this.panelVuelosDisponibles.add(this.crearPanelResultadoVueloIdaVuelta(),
				VentanaEmpleado.VUELOS_DISPONIBLES_BUSQUEDA_RESULTADO_IDA_VUELTA);
		this.panelVuelosDisponibles.add(this.crearPanelReservaSoloIda(),
				VentanaEmpleado.VUELOS_DISPONIBLES_RESERVA_SOLO_IDA);
		this.panelVuelosDisponibles.add(this.crearPanelReservaIdaVuelta(),
				VentanaEmpleado.VUELOS_DISPONIBLES_RESERVA_IDA_VUELTA);
		this.panelVuelosDisponibles.add(this.crearPanelResultadoReservaSoloIda(),
				VentanaEmpleado.VUELOS_DISPONIBLES_RESERVA_RESULTADO_SOLO_IDA);
		this.panelVuelosDisponibles.add(this.crearPanelResultadoReservaIdaVuelta(),
				VentanaEmpleado.VUELOS_DISPONIBLES_RESERVA_RESULTADO_IDA_VUELTA);
	...
	/**
	 * Muestra la pantalla de Busqueda de Vuelos Disponibles y limpia las busquedas
	 * previas
	 */
	...
	/*
	 * Recupera un vuelo de una lista en base el nroVuelo. Si no lo encuentra
	 * retorna null
	 */
	...
	/**
	 * Crea un String con HTML de la información del vuelo proporcionado para ser
	 * usado en un JLabel y que lo muestre en multiples lineas.
	 * 
	 * @param vuelo
	...
	private String setlblVueloSeleccion(InstanciaVueloBean vuelo) {
		if (Objects.isNull(vuelo)) {
			...
			String s = "<html>" + VentanaEmpleado.TABLA_VUELOS_DISPONIBLES_NRO_VUELO + ": " + vuelo.getNroVuelo()
					+ "<br>" + VentanaEmpleado.TABLA_VUELOS_DISPONIBLES_AEROPUERTO_SALIDA + ": "
					+ vuelo.getAeropuertoSalida() + "<br>" + VentanaEmpleado.TABLA_VUELOS_DISPONIBLES_HORA_SALIDA + ": "
					+ vuelo.getHoraSalida().toString() + "<br>"
					+ VentanaEmpleado.TABLA_VUELOS_DISPONIBLES_AEROPUERTO_LLEGADA + ": " + vuelo.getAeropuertoLlegada()
					+ "<br>" + VentanaEmpleado.TABLA_VUELOS_DISPONIBLES_HORA_LLEGADA + ": "
					+ vuelo.getHoraLlegada().toString() + "<br>" + VentanaEmpleado.TABLA_VUELOS_DISPONIBLES_MODELO_AVION
					+ ": " + vuelo.getModelo() + "<br>" + VentanaEmpleado.TABLA_VUELOS_DISPONIBLES_TIEMPO_ESTIMADO
					+ ": " + vuelo.getTiempoEstimado().toString() + "</html>";
			return s;
		}
	}
	...
	private String setlblDetalleSeleccion(DetalleVueloBean detalle) {
		if (Objects.isNull(detalle)) {
			...
			String s = "<html>" + VentanaEmpleado.TABLA_DETALLE_VUELOS_DISPONIBLES_CLASE + ": " + detalle.getClase()
					+ "<br>" + VentanaEmpleado.TABLA_DETALLE_VUELOS_DISPONIBLES_PRECIO + ": "
					+ String.valueOf(detalle.getPrecio()) + "<br>"
					+ VentanaEmpleado.TABLA_DETALLE_VUELOS_DISPONIBLES_ASIENTOS_DISPONIBLES + ": "
					+ String.valueOf(detalle.getAsientosDisponibles()) + "</html>";
			return s;
		}
	}
	...
	private String setlblPasajeroSeleccion(PasajeroBean pasajero) {
		if (Objects.isNull(pasajero)) {
			...
			String s = "<html>" + "Apellido: " + pasajero.getApellido() + "<br>" + "Nombre: " + pasajero.getNombre()
					+ "<br>" + pasajero.getTipoDocumento() + ":" + String.valueOf(pasajero.getNroDocumento()) + "<br>"
					+ "Nacionalidad: " + pasajero.getNacionalidad() + "<br>" + "Dirección: " + pasajero.getDireccion()
					+ "<br>" + "Telefono: " + pasajero.getTelefono() + "</html>";
			return s;
		}
	}
	...
	private String setlblResultadoPasajero(PasajeroBean pasajero) {
		if (Objects.isNull(pasajero)) {
			...
			String s = "<html>" + "<strong><span>" + "Datos del Pasajero" + "</span></strong><br><br>" + "Apellido: "
					+ pasajero.getApellido() + "<br>" + "Nombre: " + pasajero.getNombre() + "<br>"
	...
	private String setlblResultadoReserva(ReservaBean reserva) {
		...
		if (Objects.isNull(reserva)) {
			...
			s = "<html>" + "<strong><span style='color:blue;'>" + reserva.getEstado() + "</span></strong><br><br>"
					+ "Número: " + reserva.getNumero() + "<br>" + "Fecha: " + reserva.getFecha().toString() + "<br>"
					+ "Vencimiento: " + reserva.getVencimiento().toString() + "<br>" + "Registrada por el empleado: ["
					+ reserva.getEmpleado().getLegajo() + "] " + reserva.getEmpleado().getApellido() + ", "
					+ reserva.getEmpleado().getNombre() + "<br>" + "</html>";
		}
		return s;
	}

	/**
	 * Crea el panel donde se muestra el resultado de la busqueda de vuelos
	 * disponibles
	 * 
	 * @return Panel
	 */
	private Component crearPanelResultadoVueloIda() {
		...
		/*
		 * Label que diga Vuelos de IDA disponibles Tabla de Vuelos IDA - Tabla de
		 * Detalle de Vuelos IDA
		 * 
		...
		tablaDetalleVuelosDisponiblesIda.getSelectionModel().addListSelectionListener(new ListSelectionListener() {
			...
			public void valueChanged(ListSelectionEvent e) {
				if (!e.getValueIsAdjusting()) { // Porque sino se dispara multiples veces
					logger.debug(
							"se selecciono una clase o se cambio la seleccion de vuelo y se actualizaron las clases (deseleccion)");
					...
					if (tablaDetalleVuelosDisponiblesIda.getSelectedRow() != -1) {
						String clase = (String) tablaDetalleVuelosDisponiblesIda.getValueAt(
								...
								modeloTablaDetalleVuelosDisponiblesIda
										.findColumn(TABLA_DETALLE_VUELOS_DISPONIBLES_CLASE));
	...
	private Component crearPanelResultadoVueloIdaVuelta() {
		 ...
		 * Detalle de Vuelos IDA
		 * 
		 * Label que diga Vuelos de VUELTA disponibles Tabla de Vuelos VUELTA - Tabla de
		 * Detalle de Vuelos VUELTA
		 * 
		 * Botonera Volver - Reservar
		 */
				...
				if (!e.getValueIsAdjusting()) { // Porque sino se dispara multiples veces
					...
					if (tablaDetalleVuelosDisponiblesVuelta.getSelectedRow() != -1) {
						String clase = (String) tablaDetalleVuelosDisponiblesVuelta.getValueAt(
								...
								modeloTablaDetalleVuelosDisponiblesVuelta
										.findColumn(TABLA_DETALLE_VUELOS_DISPONIBLES_CLASE));
	...
	private JTable crearTabla(DefaultTableModel model) {
		...
		tabla.getSelectionModel().setSelectionMode(ListSelectionModel.SINGLE_SELECTION); // Solo puede seleccionar un
																							// item
		return tabla;
	}
